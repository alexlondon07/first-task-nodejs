...
app.post("/create-login-action", (req, res) => {
    ...
    if (response != null) {
        res.render("index", {
        tituloWeb: "App Matriculas cursos",
        msgNotificationGreen:
            "El usuario " +
            loginData.nombre +
            " se ha creado de forma correcta por favor inicia session"
        });
    } else {
        res.render("create-login", {
        tituloWeb: "App Matriculas cursos",
        msgNotification:
            "El usuario que intentas crear ya existe en base de datos"
        });
    }
});
...
app.post("/login", (req, res) => {
    ...
    let response = login.buscarLogin(loginRequest);
    sess = req.session;
